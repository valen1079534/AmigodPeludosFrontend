...
function BuscarMascota() {
    ...
    const SolicitudMascota = async () => {
        try {
            const data = {
                fk_dog_adop: dogId,
                fk_users: idUser
            };
            await axiosClient.post(`/dog/solicitud_dog`, data).then((response) => {
                if (response.status === 200) {
                    Alert.alert('SOLICITUD ENVIADA CON EXITO');
                } else {
                    Alert.alert('LA SOLICITUD FUE RECHAZADA');
                }
            });
        } catch (error) {
            console.log('Error del servidor para registrar solicitud' + error);
        }
    };
            ...
            <View style={styles.card}>
                {mascota ? (
                    <>
                        ...
                        <TouchableOpacity style={styles.button} onPress={SolicitudMascota}>
